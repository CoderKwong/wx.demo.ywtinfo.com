;(function ($) {
    ...
    function tcst() {
        ...
        $('#ListHalf li').bind('touchstart touchmove touchend', function (e) {
            if (e.type == 'touchstart') {
                $(this).addClass('touch');
            } else if (e.type == 'touchend') {
                $(this).removeClass('touch');
    ...
    $('#add_f').click(function () {
        ...
        if ($(this).hasClass('liked')) {
            var _this = $(this);
            $.post("http://m.yiwubaike.com/service/removeLike", {timestamp: YYToken.timestamp, token: YYToken.token, postid: commentId2}, function () {
                _this.removeClass('liked');
                _this.find('.lf span').html(i - 1);
            });
        } else {
            var _this = $(this);
            $.post("http://m.yiwubaike.com/service/addLike", {timestamp: YYToken.timestamp, token: YYToken.token, postid: commentId2}, function () {
        ...
        $('.relationship a').bind('click', function () {
            ...
            if (_this.hasClass('need_login')) {
                ...
                if ($(this).hasClass('add_friend') && $(this).hasClass('need_login') == false) {
                    $.post("http://m.yiwubaike.com/service/addFollow", {timestamp: YYToken.timestamp, token: YYToken.token, uid: $(this).attr('data-uid')}, function () {
                        _this.removeClass('add_friend').addClass('remove_friend');
                    });
                } else if ($(this).hasClass('remove_friend') && $(this).hasClass('need_login') == false) {
                    $.post("http://m.yiwubaike.com/service/removeFollow", {timestamp: YYToken.timestamp, token: YYToken.token, uid: $(this).attr('data-uid')}, function () {
    ...
    $('.ct li').click(function () {
        if (document.getElementById('add_favorites_choose')) {
            ...
            if ($(this).hasClass('a_selected') == false) {
                $.post("http://m.yiwubaike.com/service/addLike", {timestamp: YYToken.timestamp, token: YYToken.token, postid: commentId2, cid: d}, function () {
            ...
            if ($(this).hasClass('a_selected') && $(this).find('.s').attr('class') == undefined) {
                var _this = $(this);
                dt[1] = '';
                $.get('http://m.yiwubaike.com/list/', {fid: dt[0], owner: dt[1], sort: dt[2], search: search_value}, function (data) {
        ...
        if ($('.post_comment_content').hasClass('show')) {
            if (reply_lock == false) {
                if ($('#post_comment_content').val() != '') {
                    $.post("http://m.yiwubaike.com/service/postnewComment", {timestamp: YYToken.timestamp, token: YYToken.token, postid: commentId2, content: $('#post_comment_content').val()}, function (data) {
                ...
                if ($('#post_comment_content').val() != '') {
                    $.post("http://m.yiwubaike.com/service/postnewComment", {timestamp: YYToken.timestamp, token: YYToken.token, postid: commentId2, postcid: _cid, postuid: comment_id, content: $('#post_comment_content').val()}, function (data) {
    ...
    $('.remove_msg').click(function () {
        ...
        $('.delete_check_sure').click(function () {
            if (rmsLock) {
                rmsLock = false;
                $.post("http://m.yiwubaike.com/service/delMessage", {timestamp: YYToken.timestamp, token: YYToken.token, id: rmsId}, function () {
                    $('#delete_check').removeClass('show');
    ...
    $('.reply_msg_post').click(function () {
        if ($(this).hasClass('checked')) {
            $.post("http://m.yiwubaike.com/service/sendSMS", {timestamp: YYToken.timestamp, token: YYToken.token, content: $('#reply_msg_area').val(), nickname: reply_nickname}, function () {
                $.get("http://m.yiwubaike.com/sms", {id: reply_post_id}, function (data) {
...
//            $('.alist').openNewFrame();
            $('#container').bind('scroll', function () {
                ...
                if (t >= ($('#list').height() - $(window).height()) && scLock == true) {
                    now_page++;
                    scLock = false;
                    $.get('http://m.yiwubaike.com/list/', {page: now_page, fid: dt[0], owner: dt[1], sort: dt[2], search: search_value}, function (data) {
                        var v = $(data).find('#list ul').html();
                        $('#list ul').append(v);
    ...
    $('.more_comment').click(function () {
        $.post("http://m.yiwubaike.com/service/getcomment", {timestamp: YYToken.timestamp, token: YYToken.token, lastid: $('#comment li').eq($('#comment li').length - 1).attr('postdata'), postid: commentId2}, function (data) {
        ...
        $(window).bind('load', function () {
            ...
            $('#container').bind('scroll', function () {
                ...
                if (t >= ($('#designer_list').height() - $(window).height()) && scLock == true) {
                    designers_page++;
                    scLock = false;
                    $.get('http://m.yiwubaike.com/designers/', {page: designers_page, sort: designers_sort}, function (data) {
    ...
    $('.add_fans').click(function () {
        var _this = $(this);
        $.post("http://m.yiwubaike.com/service/addFollow", {timestamp: YYToken.timestamp, token: YYToken.token, uid: _this.attr('data-uid')}, function () {
    ...
    $('.remove_fans').click(function () {
        var _this = $(this);
        $.post("http://m.yiwubaike.com/service/removeFollow", {timestamp: YYToken.timestamp, token: YYToken.token, uid: _this.attr('data-uid')}, function () {
    ...
    $('.send_msg_to').click(function () {
        if ($(this).hasClass('checked')) {
            $.post("http://m.yiwubaike.com/service/sendSMS", {timestamp: YYToken.timestamp, token: YYToken.token, content: $('#send_msg_content').val(), nickname: send_me}, function () {
    ...
    $('.login_submit').click(function () {
        $.post("http://m.yiwubaike.com/service/loginUser", {timestamp: YYToken.timestamp, token: Y0YToken.token, email: $('.login_user input').val(), password: $('.login_password input').val()}, function () {
    ...
    $.fn.openNewFrame = function () {
        $(this).click(function () {
            var _thisv = $(this).attr('vhref'), _thist = $(this).find('.info h4').html();
            if (document.getElementById('newwrap')) {
    ...
    function replyThis() {
        ...
        $('.add_newpost_post').click(function () {
            if (reply_lock == true && isReply == true && $('.newpost_w_t textarea').val() != '') {
                reply_lock = false;
                $.post("http://m.yiwubaike.com/service/postnewComment", {timestamp: YYToken.timestamp, token: YYToken.token, postid: commentId2, postcid: _cid, postuid: comment_id, content: $('.newpost_w_t textarea').val()}, function (data) {
                    var ddd = '<div class="quote"><div class="pd10">' + _ct + '</div></div>';
            ...
            if (reply_lock == true && isReply == false && $('.newpost_w_t textarea').val() != '') {
                reply_lock = false;
                $.post("http://m.yiwubaike.com/service/postnewComment", {timestamp: YYToken.timestamp, token: YYToken.token, postid: commentId2, content: $('.newpost_w_t textarea').val()}, function (data) {
    ...
    var editAlbum = (function () {
        ...
        $('.edit_this').click(function () {
            ...
            $('.edit_album_sure').click(function () {
                $.post("http://m.yiwubaike.com/service/editUserCat", {timestamp: YYToken.timestamp, token: YYToken.token, id: pid, title: $('.edit_album_input').val()}, function (data) {
                    _this.parent().parent().parent().find('dt .pd5').html($('.edit_album_input').val());
                    $('#edit_album').removeClass('show');
                }, 'json');
            });
            $('.edit_album_delete').click(function () {
                $.post("http://m.yiwubaike.com/service/removeUserCat", {timestamp: YYToken.timestamp, token: YYToken.token, id: pid}, function (data) {
    ...
    $('.created_cate_add').click(function () {
        if ($('.created_cate_ipt input').val() != '') {
            ...
            $.post("http://m.yiwubaike.com/service/addUserCat", {timestamp: YYToken.timestamp, token: YYToken.token, fid: $('.created_cate_ipt input').val()}, function (data) {
